...
## different habitats
    ...
    habitat = ifelse(grepl('^hc|^kc', location), 'Groyne field', habitat), ## check this one, it is currently not working good!!
    habitat = ifelse(grepl('', location), 'stagnant water', habitat),
    habitat = ifelse(grepl('', location), 'flowing water', habitat),
  ) %>% 
...
# group_by(location ,species, date) %>% 
  gather(value = 'percentage', key= 'food', c(16:43)) %>% 
  mutate(
